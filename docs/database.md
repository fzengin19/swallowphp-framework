...
- **PostgreSQL** (`pgsql`) — **not currently supported**. SQL generated by the
  query builder is backtick-quoted throughout (`src/Database/Database.php`
  quotes identifiers with backticks); PostgreSQL identifiers use double
  quotes, so backtick-quoted SQL fails against a real Postgres server. Do
  not configure `database.default` to `pgsql` until identifier quoting is
  updated.
